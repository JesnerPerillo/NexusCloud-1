...
const Card = ({ course, onClick }) => {
  return (
    <div className="p-2 px-5 gap-5 flex mb-10 justify-around flex-col sm:flex-row">
      <div className="w-full rounded-4xl h-[30rem] drop-shadow-3xl card sm:w-68 sm:h-80 hover:cursor-pointer hover:scale-105 transition-transform" onClick={onClick}>
        <div className="h-3/5 rounded-t-3xl bg-white overflow-hidden" >
          <img src={course.imgSrc} alt="CCNA Image" className={`w-full h-full ${["Certified Cisco Network Associate (CCNA 200-301)", "3-Day VMware vSphere 8.0 Data Center Virtualization", "IT Support Admin Engineer by automating Windows Server", "Microsoft Certified: Security Operations Analyst Associate (SC-200)", "Microsoft Certified: Identity and Access Administrator Associate (SC-300)", "Facebook Ads Strategies 2025"].includes(course.title) ? "object-cover" : "object-contain"}`}/>
        </div>
